...
BackendPlugin::FormatInfo BackendPlugin::formatInfo( const QString& codecName )
{
    ...
    else if( codecName == "m4a/alac" )
    {
        info.priority = 80;
        info.lossless = true;
        info.description = tr("Apple Lossless Audio Codec (ALAC) is a lossless audio format. It is widely used by Apple Inc.<br>In soundKonverter \"m4a/alac\" refers to a MPEG-4 audio file encoded with ALAC.<br><br><a href=\"http://en.wikipedia.org/wiki/Apple_Lossless\">http://en.wikipedia.org/wiki/Apple Lossless</a>");
        info.mimeTypes.append( "audio/mp4" );
        info.mimeTypes.append( "audio/x-m4a" );
        info.extensions.append( "m4a" );
        info.extensions.append( "f4a" );
    ...
    {
        // info.description = tr("mp4 files are video files with an aac encoded audio stream. It is widely used by Apple Inc.");
    ...
    {
        info.description = tr("Ogg Vorbis is a lossy and popular audio format.<br>It is free of any legal restrictions.<br><br><a href=\"http://en.wikipedia.org/wiki/Vorbis\">http://en.wikipedia.org/wiki/Vorbis</a>");
    ...
    {
        info.description = tr("MPEG-1 or MPEG-2 Audio Layer III (MP3) is the most popular lossy audio format.<br>It is supported by virtually all audio players even though it has some legal restrictions.<br><br><a href=\"http://en.wikipedia.org/wiki/MP3\">http://en.wikipedia.org/wiki/MP3</a>");
    ...
    {
        info.lossless = true;
        info.description = tr("Free Lossless Audio Codec (FLAC) is a lossless audio format.<br>It is free of any legal restrictions.<br><br><a href=\"http://en.wikipedia.org/wiki/FLAC\">http://en.wikipedia.org/wiki/FLAC</a>");
    ...
    {
        info.description = tr("Windows Media Audio (WMA) is a lossy audio format developed by Microsoft.<br><br><a href=\"http://en.wikipedia.org/wiki/Windows_Media_Audio\">http://en.wikipedia.org/wiki/Windows Media Audio</a>");
        info.mimeTypes.append( "audio/x-ms-wma" );
        info.extensions.append( "wma" );
    }
    else if( codecName == "ac3" )
    {
        info.description = tr("AC3 or AC-3 is part of the Dolby Digital format family.<br>It was designed to be used for movies (e.g. DVDs) and supports surround sound.<br><br><a href=\"http://en.wikipedia.org/wiki/Dolby_Digital\">http://en.wikipedia.org/wiki/Dolby Digital</a>");
...
//     {
//         info.lossless = false;
//         info.description = tr("Dolby Digital Plus (Enhanced AC-3) is an advanced version of AC-3 for use on Blu-Ray discs.");
//         info.mimeTypes.append( "audio/eac3" );
//         info.extensions.append( "eac3" );
//     }
    else if( codecName == "mp2" )
    {
        info.description = tr("MPEG-1 or MPEG-2 Audio Layer II (MP2) is an old lossy audio format.<br><br><a href=\"http://en.wikipedia.org/wiki/MPEG-1_Audio_Layer_II\">http://en.wikipedia.org/wiki/MPEG-1 Audio Layer II</a>");
...
//     {
//         info.lossless = false;
//         info.description = tr("Sonic");
//     }
//     else if( codecName == "sonicls" )
//     {
//         info.lossless = true;
//         info.description = tr("Sonic Lossless");
//     }
    else if( codecName == "als" ) // TODO mime type, etc.
    {
        info.lossless = true;
        // info.description = tr("MPEG-4 Audio Lossless Coding"); // http://en.wikipedia.org/wiki/Audio_Lossless_Coding
        info.mimeTypes.append( "audio/mp4" );
        info.extensions.append( "mp4" );
    }
    else if( codecName == "amr nb" )
    {
        ...
        info.description = tr("Adaptive Multi-Rate Narrow-Band (AMR NB) is mainly used for speech compression for mobile communication.<br><br><a href=\"http://en.wikipedia.org/wiki/Adaptive_Multi-Rate_audio_codec\">http://en.wikipedia.org/wiki/Adaptive Multi-Rate audio codec</a>");
    ...
    {
        info.inferiorQuality = true;
        info.description = tr("Adaptive Multi-Rate Wide-Band (AMR WB) is an advanced version of AMR NB which uses a higher data rate resulting in a higher quality.<br>Still it should be used only for speech compression.<br><br><a href=\"http://en.wikipedia.org/wiki/Adaptive_Multi-Rate_Wideband\">http://en.wikipedia.org/wiki/Adaptive Multi-Rate Wideband</a>");
    ...
    {
        info.lossless = true;
        info.description = tr("Monkey's Audio is a lossless audio format.<br><br><a href=\"http://en.wikipedia.org/wiki/Monkey's_Audio\">http://en.wikipedia.org/wiki/Monkey's Audio</a>");
    ...
    {
        info.inferiorQuality = true;
        info.description = tr("Speex is a lossy audio format designed for speech encoding.<br>It is free of any legal restrictions. Since the release of Opus, Speex is considered obsolete.<br><br><a href=\"http://en.wikipedia.org/wiki/Speex\">http://en.wikipedia.org/wiki/Speex<a/>");
    ...
    {
        // info.description = tr("MPEG-1 Audio Layer I very old and lossy file format."); // http://en.wikipedia.org/wiki/MP1
        info.mimeTypes.append( "audio/mpeg" );
    ...
    {
        info.description = tr("Musepack (MPC) is a free and lossy file format based on mp2 and optimized for high quality.<br><br><a href=\"http://en.wikipedia.org/wiki/Musepack\">http://en.wikipedia.org/wiki/Musepack<a/>");
    ...
    {
        info.lossless = true;
        info.description = tr("Shorten (SHN) is a free and lossless audio format.<br><br><a href=\"http://en.wikipedia.org/wiki/Shorten\">http://en.wikipedia.org/wiki/Shorten</a>");
...
//     {
//         info.lossless = true;
//         info.description = tr("Meridian Lossless Packing is an old propritary lossless audio format."); // http://en.wikipedia.org/wiki/Meridian_Lossless_Packing
//     }
//     else if( codecName == "truehd" )
//     {
//         info.lossless = true;
//         info.description = tr("Dolby TrueHD is a lossless audio format based on mlp for use on Blu-Ray discs."); // http://en.wikipedia.org/wiki/Dolby_TrueHD
//     }
//     else if( codecName == "truespeech" )
//     {
//         info.lossless = false;
//         info.description = tr("Truespeech is a propritary speech codec for low bitrates."); // http://en.wikipedia.org/wiki/Truespeech
//     }
    else if( codecName == "tta" )
    {
        info.lossless = true;
        info.description = tr("True Audio (TTA) is a free and lossless audio format.<br><br><a href=\"http://en.wikipedia.org/wiki/TTA_(codec)\">http://en.wikipedia.org/wiki/TTA (codec)<a/>");
    ...
    {
        info.lossless = true;
        info.description = tr("WavPack (WV) is a free and lossless audio format.<br><br><a href=\"http://en.wikipedia.org/wiki/WavPack\">http://en.wikipedia.org/wiki/WavPack<a/>");
    ...
    {
        // info.description = tr("Real Media Audio is a proprietary and lossy codec.");
    ...
    {
        // info.description = tr("3GP is a audio/video container format for mobile devices."); // http://de.wikipedia.org/wiki/3gp
    ...
    {
        // info.description = tr("Real Media is a proprietary and lossy codec.");
    ...
    {
        info.inferiorQuality = true;
        // info.description = tr("midi is a very old sound file format, that doesn't encode audio waves but stores instrument timings.");
    ...
    {
        info.inferiorQuality = true;
        // info.description = tr("Amiga-SoundTracker-Audio is a very old sound file format, that doesn't encode audio waves but stores instrument timings.");
    ...
    {
        info.inferiorQuality = true;
        // info.description = tr("Black & White audio files where used for the game Black & White and contain an MP2 stream.");
        info.extensions.append( "sad" );
    }
    else if( codecName == "8svx" )
    {
        info.inferiorQuality = true;
        info.description = tr("8-Bit Sampled Voice (8SVX) is an audio file format standard developed by Electronic Arts for the Commodore-Amiga computer series.<br><br><a href=\"http://en.wikipedia.org/wiki/8SVX\">http://en.wikipedia.org/wiki/8SVX</a>");
...
//     {
//         info.lossless = true;
//         info.description = tr("AIFF-C is a compressed version of AIFF. There are different compression types available.");
//         info.mimeTypes.append( "audio/x-aiff" );
//         info.mimeTypes.append( "audio/aiff" );
...
bool BackendPlugin::kill( int id )
{
    for( int i=0; i<backendItems.size(); i++ )
    {
        if( backendItems.at(i)->id == id && backendItems.at(i)->process != 0 )
        {
            ...
            emit log( id, "<pre>\t" + tr("Killing process on user request") + "</pre>" );
            return true;
        }
    }
    return false;
...
QString BackendPlugin::standardMessage( const QString& type, const QStringList& arguments ) const
{
    if( type == "encode_codec,backend" )
    {
        ...
        return tr("In order to encode %1 files, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(1));
    }
    if( type == "decode_codec,backend" )
    {
        ...
        return tr( "In order to decode %1 files, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(1) );
    }
    if( type == "transcode_codec,backend" )
    {
        ...
        return tr( "In order to transcode %1 files, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(1));
    }
    if( type == "replygain_codec,backend" )
    {
        ...
        return tr( "In order to calculate Replay Gain tags for %1 files, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(1) );
    }
    if( type == "install_opensource_backend" )
    {
        ...
        return tr( "'%1' should be shipped with your distribution.").arg(arguments.at(0));
    }
    if( type == "install_patented_backend" )
    {
        ...
        return tr( "Since '%1' includes patented codecs, it may not be included in the default installation of your distribution. Many distributions offer '%1' in an additional software repository.").arg( arguments.at(0) );
    }
    if( type == "install_website_backend,url" )
    {
        ...
        return tr( "You can download '%1' at %2").arg( arguments.at(0)).arg(arguments.at(1) );
    }
    if( type == "filter,backend" )
    {
        ...
        {
            return tr( "In order to use the filter %1, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(2) );
        }
        else
        {
            return tr( "In order to use the filters %1, you need to install '%2'.").arg(arguments.at(0)).arg(arguments.at(2) );
        }
    }
