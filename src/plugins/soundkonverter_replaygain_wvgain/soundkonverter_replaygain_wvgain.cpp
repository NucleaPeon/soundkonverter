...
QList<ReplayGainPipe> soundkonverter_replaygain_wvgain::codecTable()
{
    ...
    newPipe.problemInfo = standardMessage( "replaygain_codec,backend", "wavpack", "wvgain" ) + "\n" + tr( "'%1' is usually in the package '%2' which you can download at %3", QString("wvgain"), QString("wavpack"), QString("http://www.wavpack.com") );
...
float soundkonverter_replaygain_wvgain::parseOutput( const QString& output )
{
    ...
    QRegularExpressionMatch match = reg.match(ouput);
    if( match.hasMatch() )
    {
        return (float)match.capture(1).toInt();
    }
