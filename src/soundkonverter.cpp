      ...
      autoclose( false )
{
    ...
    logger->log( 1000, tr("This is soundKonverter %1",MAJOR) );

    logger->log( 1000, "\n" + tr("Compiled with TagLib %1.%2.%3",TAGLIB_MAJOR_VERSION,TAGLIB_MINOR_VERSION,TAGLIB_PATCH_VERSION) );
...
void soundKonverter::showSystemTray()
{
    ...
    systemTray->setToolTip( "soundkonverter", tr("Waiting"), "" );
}
...
void soundKonverter::setupActions()
{
    ...
    logviewer->setText(tr("View logs..."));
    logviewer->setIcon(QIcon("view-list-text"));
    ...
    replaygainscanner->setText(tr("Replay Gain tool..."));
    replaygainscanner->setIcon(QIcon("soundkonverter-replaygain"));
    ...
    aboutplugins->setText(tr("About plugins..."));
    aboutplugins->setIcon(QIcon("preferences-plugin"));
...
void soundKonverter::startupChecks()
{
    ...
    if( config->data.app.configVersion < 1001 )
    {
        if( QFile::exists(QDir::homePath()+"/.kde4/share/kde4/services/ServiceMenus/convert_with_soundkonverter.desktop") )
        {
            ...
            logger->log( 1000, tr("Removing old file: %1",QDir::homePath()+"/.kde4/share/kde4/services/ServiceMenus/convert_with_soundkonverter.desktop") );
        }
        if( QFile::exists(QDir::homePath()+"/.kde4/share/kde4/services/ServiceMenus/add_replaygain_with_soundkonverter.desktop") )
        {
            ...
            logger->log( 1000, tr("Removing old file: %1",QDir::homePath()+"/.kde4/share/kde4/services/ServiceMenus/add_replaygain_with_soundkonverter.desktop") );
        }
    }

    // clean up log directory
    QDir dir( QStandardPaths::locateLocal("data","soundkonverter/log/") );
    ...
    for( QStringList::Iterator it = list.begin(); it != list.end(); ++it )
    {
        if( *it != "1000.log" && (*it).endsWith(".log") )
        {
            ...
            logger->log( 1000, tr("Removing old file: %1",dir.absolutePath()+"/"+(*it)) );
        }
    }
...
void soundKonverter::conversionStarted()
{
    if( systemTray )
    {
        systemTray->setToolTip( "soundkonverter", tr("Converting") + ": 0%", "" );
    }
}
...
void soundKonverter::conversionStopped( bool failed )
{
    if( autoclose && !failed /*&& !m_view->isVisible()*/ )
        QApplication::QApplication()->quit(); // close app on conversion stop unless the conversion was stopped by the user or the window is shown

    if( systemTray )
    {
        systemTray->setToolTip( "soundkonverter", tr("Finished"), "" );
    }
}
...
void soundKonverter::progressChanged( const QString& progress )
{
    ...
    if( systemTray )
    {
            ...
            systemTray->setToolTip( "soundkonverter", tr("Converting") + ": " + progress, "" );
        #else
            systemTray->setToolTip( tr("Converting") + ": " + progress );
        #endif
    }
}
